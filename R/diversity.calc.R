...
#'
#' @section Details: The diversity indices implemented in \code{diversity.calc}
...
#'
#' @returns A list of the different diversity measures computed. \describe{
#'   \item{richness}{The number of classes in \code{x} or the richness.}
#'   \item{margalef_index}{Margalef's Richness Index (\mjseqn{D_{Margalef}})
...
diversity.calc <- function(x, base = exp(1), na.omit = TRUE) {
  ...
  out <- list(richness = length(levels(x)),
              margalef_index = margalef_index(x),
              menhinick_index = menhinick_index(x),
              berger_parker = berger_parker(x),
              berger_parker_reciprocal = berger_parker_reciprocal(x),
              simpson = simpson(x),
              gini_simpson = gini_simpson(x),
              simpson_max = simpson_max(x),
              simpson_reciprocal = simpson_reciprocal(x),
              simpson_relative = simpson_relative(x),
              simpson_evenness = simpson_evenness(x),
              shannon = shannon(x, base = base),
              shannon_max = shannon_max(x, base = base),
              shannon_relative = shannon_relative(x, base = base),
              shannon_ens = shannon_ens(x, base = base),
              heip_evenness = heip_evenness(x),
              mcintosh_diversity = mcintosh_diversity(x),
